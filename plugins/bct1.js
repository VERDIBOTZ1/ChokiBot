...
//for (let id of groups) await itsu.sendButton(id, t1 + '\n\n' + readMore + ` 「 ${bc} Group Broadcast 」`, footer, t2, t3)
for (let id of chats) await itsu.sendButton(id, t1 + '\n\n' + readMore + ` 「 ${bc} Broadcast 」`, footer, t2, t3)
...
/*
let chats = itsu.chats.all().filter(v => !v.read_only && v.message && !v.archive).map(v => v.jid)
let cc = itsu.serializeM(text ? m : m.quoted ? await m.getQuotedObj() : false || m)
let teks = text ? text : cc.text
itsu.reply(m.chat, `_Mengirim pesan broadcast ke ${chats.length} chat_`, m)
for (let id of chats) await itsu.copyNForward(id, itsu.cMod(m.chat, cc, /bc|broadcast/i.test(teks) ? teks : teks + '\n' + readMore + `\n「 ${bc} Broadcast 」`), true).catch(_=>_)
m.reply('Selesai Broadcast All Chat :)')
}
handler.help = ['broadcast','bc'].map(v => v + ' <teks>')
handler.tags = ['owner']
